...
import ubicaciodeinventario.acecoadatos.SucursalDAL; // Importar la clase RolDAL de la capa de acceso a datos
import ubicaciodeinventario.acecoadatos.BodegaDAL;; // Importar la clase UsuarioDAL de la capa de acceso a datos
import ubicaciodeinventario.appweb.utils.*; // Importar las clases SessionUser, Utilidad del paquete de utils
import ubicaciodeinventario.entidadesdenegocio.Sucursal; // Importar la clase Rol de la capa de entidades de negocio
import ubicaciodeinventario.entidadesdenegocio.Bodega; // Importar la clase Usuario de la capa de entidades de negocio
 ...
 * /Rol
 */
@WebServlet(name = "BodegaServlet", urlPatterns = {"/Bodega"})
public class BodegaServlet extends HttpServlet {
     ...
     */
    private Bodega obtenerBodega(HttpServletRequest request) {
        ...
        if (accion.equals("create") == false) { // Si la accion no es create.
            ...
            bodega.setId(Integer.parseInt(Utilidad.getParameter(request, "id", "0")));
        }
        // Obtener el parámetro nombre del request   y asignar ese valor a la propiedad Nombre de Rol.
        bodega.setNombre(Utilidad.getParameter(request, "nombre", ""));
         bodega.setDecripcion(Utilidad.getParameter(request, "descripcion", ""));
         bodega.setIdSucursal(Integer.parseInt(Utilidad.getParameter(request, "idSucursal", "0")));
         bodega.setEstatus(Byte.parseByte(Utilidad.getParameter(request, "estatus", "0")));
        if (accion.equals("index")) {  // Si accion es index.
            ...
            bodega.setTop_aux(Integer.parseInt(Utilidad.getParameter(request, "top_aux", "10")));
            ...
            bodega.setTop_aux(bodega.getTop_aux() == 0 ? Integer.MAX_VALUE : bodega.getTop_aux());
        }
    ...
    private void doGetRequestIndex(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            Bodega bodega = new Bodega(); // Crear una instancia  de la entidad de Rol.
            bodega.setTop_aux(10); // Agregar el Top_aux con el valor de 10 a la propiedad Top_aux de rol.
            ArrayList<Bodega> bodegas = BodegaDAL.buscar(bodega); // Ir a la capa de acceso a datos y buscar los registros de Rol.
            // El request.setAttribute se utiliza para enviar datos desde un servlet a un jsp.
            request.setAttribute("bodegas", bodegas); // Enviar los roles al jsp utilizando el request.setAttribute con el nombre del atributo roles.
            // Enviar el Top_aux de Rol al jsp utilizando el request.setAttribute con el nombre del atributo top_aux.
            request.setAttribute("top_aux", bodega.getTop_aux());
            // El request.getRequestDispatcher nos permite direccionar a un jsp desde un servlet.              
            request.getRequestDispatcher("Views/Bodega/index.jsp").forward(request, response); // Direccionar al jsp index de Rol.
        } catch (Exception ex) {
    ...
    private void doPostRequestIndex(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            Bodega bodega = obtenerBodega(request); // Llenar la instancia de Rol con los parámetros enviados en el request 
            ArrayList<Bodega> bodegas = BodegaDAL.buscar(bodega); // Buscar los roles que cumple con los datos enviados en el request
            request.setAttribute("bodegas", bodegas); // Enviar los roles al jsp utilizando el request.setAttribute con el nombre del atributo roles
            // Enviar el Top_aux de Rol al jsp utilizando el request.setAttribute con el nombre del atributo top_aux
            request.setAttribute("top_aux", bodega.getTop_aux());
            request.getRequestDispatcher("Views/Bodega/index.jsp").forward(request, response); // Direccionar al jsp index de Rol
    ...
    private void doGetRequestCreate(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        request.getRequestDispatcher("Views/Bodega/create.jsp").forward(request, response);
    }
    ...
    private void requestObtenerPorId(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            Bodega bodega_result = BodegaDAL.obtenerPorId(bodega);
            if (bodega_result.getId() > 0) { // Si el Id es mayor a cero.
                // Enviar el atributo rol con el valor de los datos del rol de nuestra base de datos a un jsp
                request.setAttribute("bodega", bodega_result);
            } else {
                // Enviar al jsp de error el siguiente mensaje. El Id: ? no existe en la tabla de Rol
                Utilidad.enviarError("El Id:" + bodega.getId() + " no existe en la tabla de Rol", request, response);
            }
        } catch (Exception ex) {
            // enviar al jsp de error si hay un Exception
            Utilidad.enviarError(ex.getMessage(), request, response);
    ...
    private void doGetRequestEdit(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        request.getRequestDispatcher("Views/Bodega/edit.jsp").forward(request, response);
    }
    ...
    private void doGetRequestDetails(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        request.getRequestDispatcher("Views/Bodega/details.jsp").forward(request, response);
    }
    ...
    private void doGetRequestDelete(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        request.getRequestDispatcher("Views/Bodega/delete.jsp").forward(request, response);
    }
    ...
    private void doPostRequestDelete(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            int result = BodegaDAL.eliminar(bodega);
