...
import ubicaciodeinventario.acecoadatos.EstanteDAL; // Importar la clase UsuarioDAL de la capa de acceso a datos
 ...
 * @author natalia
 */
@WebServlet(name = "EstanteServlet", urlPatterns = {"/EstanteServlet"})
public class EstanteServlet extends HttpServlet {
     ...
     */
    private Estante obtenerEstante(HttpServletRequest request) {
       ...
       Estante estante = new Estante();
        if (accion.equals("create") == false) { // Si la accion no es create.
            ...
            estante.setId(Integer.parseInt(Utilidad.getParameter(request, "id", "0")));
        }
        // Obtener el parámetro nombre del request   y asignar ese valor a la propiedad Nombre de Rol.
       estante.setNombre(Utilidad.getParameter(request, "nombre", ""));
        if (accion.equals("index")) {  // Si accion es index.
           ...
           estante.setTop_aux(Integer.parseInt(Utilidad.getParameter(request, "top_aux", "10")));
...
//           estante.setTop_aux(estante.getTop_aux() == 0 ? Integer.MAX_VALUE : estante.getTop_aux()) ESTA ES CODIGO MALO COMENTADO;
        }
    ...
    private void doGetRequestIndex(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            Estante estante = new Estante(); // Crear una instancia  de la entidad de Rol.
            estante.setTop_aux(10); // Agregar el Top_aux con el valor de 10 a la propiedad Top_aux de rol.
            ArrayList<Estante> estantes = EstanteDAL.buscar(estante); // Ir a la capa de acceso a datos y buscar los registros de Rol.
            // El request.setAttribute se utiliza para enviar datos desde un servlet a un jsp.
            request.setAttribute("estante", estantes); // Enviar los roles al jsp utilizando el request.setAttribute con el nombre del atributo roles.
            // Enviar el Top_aux de Rol al jsp utilizando el request.setAttribute con el nombre del atributo top_aux.
            request.setAttribute("top_aux", estante.getTop_aux());
    ...
    private void doPostRequestIndex(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            Estante estante = obtenerEstante(request); // Llenar la instancia de Rol con los parámetros enviados en el request 
            ArrayList<Estante> estantes = EstanteDAL.buscar(estante); // Buscar los roles que cumple con los datos enviados en el request
            request.setAttribute("estantes", estantes); // Enviar los roles al jsp utilizando el request.setAttribute con el nombre del atributo roles
            // Enviar el Top_aux de Rol al jsp utilizando el request.setAttribute con el nombre del atributo top_aux
            request.setAttribute("top_aux", estante.getTop_aux());
            request.getRequestDispatcher("Views/Rol/index.jsp").forward(request, response); // Direccionar al jsp index de Rol
        } catch (Exception ex) {
    ...
    private void doGetRequestCreate(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        request.getRequestDispatcher("Views/Estante/create.jsp").forward(request, response);
    }
    ...
    private void requestObtenerPorId(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
           ...
           Estante estante_result = EstanteDAL.obtenerPorId(estante);
            if (estante_result.getId() > 0) { // Si el Id es mayor a cero.
                // Enviar el atributo rol con el valor de los datos del rol de nuestra base de datos a un jsp
                request.setAttribute("estante", estante_result);
            } else {
                // Enviar al jsp de error el siguiente mensaje. El Id: ? no existe en la tabla de Rol
                Utilidad.enviarError("El Id:" + estante.getId() + " no existe en la tabla de Estante", request, response);
            }
        } catch (Exception ex) {
            // enviar al jsp de error si hay un Exception
            Utilidad.enviarError(ex.getMessage(), request, response);
    ...
    private void doGetRequestEdit(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        request.getRequestDispatcher("Views/Estante/edit.jsp").forward(request, response);
    }
    ...
    private void doGetRequestDetails(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        request.getRequestDispatcher("Views/Estante/details.jsp").forward(request, response);
    }
    ...
    private void doGetRequestDelete(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        request.getRequestDispatcher("Views/Estante/delete.jsp").forward(request, response);
    }
    ...
    private void doPostRequestDelete(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            int result = EstanteDAL.eliminar(estante);
