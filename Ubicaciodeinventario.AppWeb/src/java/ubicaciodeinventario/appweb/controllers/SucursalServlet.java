...
public class SucursalServlet extends HttpServlet {
     ...
     */
    private Sucursal obtenerSucursal(HttpServletRequest request) {
        ...
        if (accion.equals("create") == false) { // Si la accion no es create.
            ...
            sucursal.setIdSucursal(Integer.parseInt(Utilidad.getParameter(request, "id", "0")));
        }
        // Obtener el parámetro nombre del request   y asignar ese valor a la propiedad Nombre de Rol.
        sucursal.setNombre(Utilidad.getParameter(request, "nombre", ""));
        if (accion.equals("index")) {  // Si accion es index.
            ...
            sucursal.setTop_Aux(Integer.parseInt(Utilidad.getParameter(request, "top_aux", "10")));
            ...
            sucursal.setTop_Aux(sucursal.getTop_Aux() == 0 ? Integer.MAX_VALUE : sucursal.getTop_Aux());
        }
    ...
    private void doGetRequestIndex(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            Sucursal sucursal = new Sucursal(); // Crear una instancia  de la entidad de Rol.
            sucursal.setTop_Aux(10); // Agregar el Top_aux con el valor de 10 a la propiedad Top_aux de rol.
            ArrayList<Sucursal> sucursales = SucursalDAL.buscar(sucursal); // Ir a la capa de acceso a datos y buscar los registros de Rol.
            // El request.setAttribute se utiliza para enviar datos desde un servlet a un jsp.
            request.setAttribute("sucursales", sucursales); // Enviar los roles al jsp utilizando el request.setAttribute con el nombre del atributo roles.
            // Enviar el Top_aux de Rol al jsp utilizando el request.setAttribute con el nombre del atributo top_aux.
            request.setAttribute("top_aux", sucursal.getTop_Aux());
            // El request.getRequestDispatcher nos permite direccionar a un jsp desde un servlet.              
            request.getRequestDispatcher("Views/Sucursal/index.jsp").forward(request, response); // Direccionar al jsp index de Rol.
        } catch (Exception ex) {
    ...
    private void doPostRequestIndex(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            Sucursal sucursal = obtenerSucursal(request); // Llenar la instancia de Rol con los parámetros enviados en el request 
            ArrayList<Sucursal> sucursales = SucursalDAL.buscar(sucursal); // Buscar los roles que cumple con los datos enviados en el request
            request.setAttribute("sucursales", sucursales); // Enviar los roles al jsp utilizando el request.setAttribute con el nombre del atributo roles
            // Enviar el Top_aux de Rol al jsp utilizando el request.setAttribute con el nombre del atributo top_aux
            request.setAttribute("top_aux", sucursal.getTop_Aux());
            request.getRequestDispatcher("Views/Sucursal/index.jsp").forward(request, response); // Direccionar al jsp index de Rol
    ...
    private void requestObtenerPorId(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            Sucursal sucursal_result = SucursalDAL.obtenerPorId(sucursal);
            if (sucursal_result.getIdSucursal() > 0) { // Si el Id es mayor a cero.
                // Enviar el atributo rol con el valor de los datos del rol de nuestra base de datos a un jsp
                request.setAttribute("rol", sucursal_result);
            } else {
                // Enviar al jsp de error el siguiente mensaje. El Id: ? no existe en la tabla de Rol
                Utilidad.enviarError("El Id:" + sucursal.getIdSucursal() + " no existe en la tabla de Rol", request, response);
            }
        } catch (Exception ex) {
            // enviar al jsp de error si hay un Exception
            Utilidad.enviarError(ex.getMessage(), request, response);
    ...
    private void doPostRequestEdit(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            int result = SucursalDAL.modificar(sucursal);
    ...
    private void doPostRequestDelete(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            int result = SucursalDAL.eliminar(sucursal);
